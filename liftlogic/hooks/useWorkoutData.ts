...
export const useWorkoutData = (isAuthenticated: boolean) => {
  ...
  const fetchDataAndSync = useCallback(async () => {
    try {
      ...
      // Ensure logs are sorted descending (newest first) for optimization
  ...
  // Group logs by exercise ID to allow O(1) retrieval.
  // Since `logs` is already pre-sorted descending, each group's array is also sorted.
  ...
  const getTodaysLogs = useCallback((id: string) => {
    ...
    const result = [];
    // Efficient early-exit iteration on pre-sorted logs (newest first)
    ...
    // Return in chronological order (oldest first) as expected by components
  ...
  const getLastSessionLogs = useCallback((id: string) => {
    ...
    // Find the first log before today to identify the last session's date
