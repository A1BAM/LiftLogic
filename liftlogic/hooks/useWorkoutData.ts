...
export const useWorkoutData = (isAuthenticated: boolean) => {
  ...
  // Memoized Map of logs by exerciseId for O(1) retrieval
  const logsByExercise = useMemo(() => {
    const map = new Map<string, WorkoutLog[]>();
    // logs are already kept sorted or sorted on ingest
    logs.forEach(log => {
  ...
  const fetchDataAndSync = useCallback(async () => {
    try {
      ...
      // Sort logs descending on initial fetch
  ...
  const getTodaysLogs = useCallback((id: string) => {
    ...
    // logsByExercise[id] is already sorted descending [newest...oldest]
    ...
    for (const log of exerciseLogs) {
      if (log.timestamp >= startOfDay && log.timestamp < endOfDay) {
        ...
        // Since logs are sorted descending, once we see a log older than today, we can stop
        break;
      }
    }
    // Return in reverse (ascending) for today's view as expected by original logic
  ...
  const getLastSessionLogs = useCallback((id: string) => {
    ...
    for (const log of exerciseLogs) {
      if (log.timestamp >= startOfLastDay && log.timestamp < endOfLastDay) {
        ...
        // Optimization: stop once we're before the target session
        break;
      }
    }
    return results;
