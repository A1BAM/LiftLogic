...
export const useWorkoutData = (isAuthenticated: boolean) => {
  ...
  const fetchDataAndSync = useCallback(async () => {
    try {
      ...
      // Ensure logs are sorted newest first for optimized retrieval
  ...
  const getTodaysLogs = useCallback((id: string) => {
    ...
    // Optimized: Early exit since logs are sorted DESC
    const results = [];
    for (const log of all) {
      if (log.timestamp < startOfDay) break;
      results.push(log);
    }
    return results.reverse(); // Standardize on chronological for "Today"
  }, [getLogsForExercise]);
