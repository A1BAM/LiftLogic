...
export const useWorkoutData = (isAuthenticated: boolean) => {
  ...
  const logsByExercise = useMemo(() => {
    ...
    // Sort all logs once descending (newest first)
  ...
  const getTodaysLogs = useCallback((id: string) => {
    ...
    // Since 'all' is sorted newest first, today's logs are at the beginning.
    const todays = [];
    for (const log of all) {
      if (log.timestamp >= startOfDay) {
        todays.push(log);
      } else {
        break; // Stop once we hit a log from yesterday
      }
    }
    return todays.reverse(); // Return oldest first for display
  }, [getLogsForExercise]);
  ...
  const getLastSessionLogs = useCallback((id: string) => {
    ...
    const firstNotTodayIndex = all.findIndex(l => l.timestamp < startOfToday);
    if (firstNotTodayIndex === -1) return [];

    const lastLog = all[firstNotTodayIndex];
    const d = new Date(lastLog.timestamp);
    const startOfLastDay = new Date(d.getFullYear(), d.getMonth(), d.getDate()).getTime();

    const lastSession = [];
    for (let i = firstNotTodayIndex; i < all.length; i++) {
      if (all[i].timestamp >= startOfLastDay) {
        lastSession.push(all[i]);
      } else {
        break;
      }
    }
