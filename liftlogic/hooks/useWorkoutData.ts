...
export const useWorkoutData = (isAuthenticated: boolean) => {
  ...
  const fetchDataAndSync = useCallback(async () => {
    try {
      ...
      // Keep logs sorted descending (newest first) for optimized retrieval
  ...
  const addLog = async (exerciseId: string, weight: number, reps: number) => {
    ...
    // Prepend log to maintain descending order
  ...
  const importLogs = async (importedLogs: WorkoutLog[]) => {
    setLogs(prevLogs => {
      ...
      // Ensure logs remain sorted descending after import
  ...
  const getTodaysLogs = useCallback((id: string) => {
    ...
    // Since logs are sorted newest first, we can early exit
    ...
    return result.reverse(); // Return in chronological order for today's view
  }, [getLogsForExercise]);
