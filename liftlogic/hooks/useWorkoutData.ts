...
export const useWorkoutData = (isAuthenticated: boolean) => {
  ...
  // Memoized Map to group logs by exerciseId for O(1) retrieval
  const logsByExercise = useMemo(() => {
    const map = new Map<string, WorkoutLog[]>();

    // Sort logs once here (newest first) to avoid sorting in every getter
  ...
  /**
   * Performance: O(1) retrieval via memoized Map.
   * Result is already sorted newest first by the Map generator.
   */
  ...
  /**
   * Performance: O(K) where K is the number of logs for this exercise.
   * Uses early-exit since logs are sorted descending.
   */
  const getTodaysLogs = useCallback((id: string) => {
    ...
    // Efficiently collect today's logs using early exit
    for (const log of exerciseLogs) {
      if (log.timestamp >= endOfDay) continue; // Future dated logs
      if (log.timestamp < startOfDay) break; // Reached yesterday, stop
      result.push(log);
    }

    // App expects today's logs in ascending order (oldest first)
  ...
  const getLastSessionLogs = useCallback((id: string) => {
    ...
    // Find first log before today (already sorted newest first)
