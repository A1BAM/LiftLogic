...
export const useWorkoutData = (isAuthenticated: boolean) => {
  ...
  const importLogs = async (importedLogs: WorkoutLog[]) => {
    setLogs(prevLogs => {
      ...
      // Sort merged logs to maintain descending order
  ...
  const getLogsForExercise = useCallback((id: string) => {
    // Uses memoized Map and pre-sorted logs
  ...
  const getTodaysLogs = useCallback((id: string) => {
    ...
    // Performance: Early-exit iteration on newest-first sorted array
    for (const log of all) {
      if (log.timestamp >= startOfDay) {
        result.push(log);
      } else {
        break; // Reached logs before today
      }
    }
    // Note: getTodaysLogs is expected to return in ascending order for the daily view
  ...
  const getLastSessionLogs = useCallback((id: string) => {
    ...
    const result: WorkoutLog[] = [];
    // Performance: Early-exit iteration starting from logs before today
    for (const log of all) {
      if (log.timestamp < endOfLastDay) {
        if (log.timestamp >= startOfLastDay) {
          result.push(log);
        } else {
          break; // Reached logs before the last session day
        }
      }
    }
    return result;
