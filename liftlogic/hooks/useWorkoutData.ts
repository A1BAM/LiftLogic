...
export const useWorkoutData = (isAuthenticated: boolean) => {
  ...
  const fetchDataAndSync = useCallback(async () => {
    try {
      ...
      // Maintain logs in descending chronological order (newest first)
  ...
  const importLogs = async (importedLogs: WorkoutLog[]) => {
    setLogs(prevLogs => {
      ...
      // Re-sort after merging to maintain descending order
  ...
  const logsByExercise = useMemo(() => {
    ...
    // O(N) grouping, but since 'logs' is already sorted newest-first,
    // each group's array will also be sorted newest-first.
  ...
  const getTodaysLogs = useCallback((id: string) => {
    ...
    for (const log of exerciseLogs) {
      // Early exit if we reach logs older than today (since it's sorted newest first)
      if (log.timestamp < startOfDay) break;
      if (log.timestamp < endOfDay) {
        results.push(log);
      }
    }
    // Return in ascending order (oldest first) as per UI expectation
  ...
  const getLastSessionLogs = useCallback((id: string) => {
    ...
    let lastLogNotToday: WorkoutLog | undefined;
    for (const log of exerciseLogs) {
    ...
    const results: WorkoutLog[] = [];
    for (const log of exerciseLogs) {
      // Future logs or today's logs relative to the session we found
      if (log.timestamp >= endOfLastDay) continue;
      // Early exit if we reach logs older than the session's day
