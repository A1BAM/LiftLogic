...
export const useWorkoutData = (isAuthenticated: boolean) => {
  ...
  const getTodaysLogs = useCallback((id: string) => {
    ...
    // 'all' is sorted newest first. Early exit when we pass today's boundary.
    ...
    return results.reverse(); // Reverse to return chronological order for "today's list"
  }, [getLogsForExercise]);
  ...
  const getLastSessionLogs = useCallback((id: string) => {
    ...
    let lastLogNotToday: WorkoutLog | null = null;
    for (const log of all) {
    ...
    const results: WorkoutLog[] = [];
    for (const log of all) {
      if (log.timestamp >= startOfLastDay && log.timestamp < (startOfLastDay + 86400000)) {
        results.push(log);
