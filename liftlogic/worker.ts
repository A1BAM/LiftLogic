      ...
      if (request.method === 'DELETE') {
        ...
        if (exerciseId) {
          if (typeof exerciseId !== 'string') {
            return new Response(JSON.stringify({ error: "Invalid exerciseId type" }), {
              status: 400,
              headers
            });
          }
        ...
        if (id) {
          if (typeof id !== 'string') {
            return new Response(JSON.stringify({ error: "Invalid id type" }), {
              status: 400,
              headers
            });
          }
        ...
        return new Response(JSON.stringify({ error: "Missing ID or Exercise ID" }), {
          status: 400,
          headers
        });
      }
