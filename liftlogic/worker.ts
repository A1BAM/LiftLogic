...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // Security Check: Verify Bearer Token (Mandatory if not OPTIONS)
    if (request.method !== 'OPTIONS') {
      if (!env.TARGET_HASH) {
        logger.error("TARGET_HASH not set. API is incorrectly configured.");
        return new Response(JSON.stringify({ error: "Internal Server Error: Authentication configuration missing" }), {
          status: 500,
          headers: headers
        });
      }
    ...
    try {
      ...
      if (request.method === 'POST') {
        ...
        // Security: Prevent DoS/Resource Exhaustion via oversized notes
        if (notes && typeof notes === 'string' && notes.length > 500) {
          return new Response(JSON.stringify({ error: "Notes field too long (max 500 chars)" }), { status: 400, headers });
        }
