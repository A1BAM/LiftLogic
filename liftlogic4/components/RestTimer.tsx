    ...
    <div className="fixed bottom-4 left-4 right-4 z-50 flex justify-center animate-in slide-in-from-bottom-5">
      <div className="bg-slate-900 border border-slate-700 shadow-2xl rounded-2xl p-4 flex items-center gap-4 w-full max-w-sm">
        ...
        <div className="flex flex-col gap-1">
           <button 
             onClick={() => onAdd(30)}
             className="p-1 bg-slate-800 hover:bg-slate-700 text-white rounded border border-slate-700"
           >
             <Plus size={14} />
           </button>
           <button 
        ...
        <button 
          onClick={onCancel}
          className="p-2 hover:bg-slate-800 rounded-full text-slate-500 hover:text-white transition-colors ml-2"
        >
          <X size={20} />
        </button>
      </div>
